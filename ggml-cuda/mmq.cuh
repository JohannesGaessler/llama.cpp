...
#pragma unroll
        for (int i0 = 0; i0 < mmq_y; i0 += WARP_SIZE) {
            ...
            sum[j0/nwarps*mmq_y/WARP_SIZE + i0/WARP_SIZE] += vec_dot_q3_K_q8_1_impl_mmq(
                &x_qs[i*(QR3_K*WARP_SIZE + 1) + k0*QR3_K], &y_qs[j*MMQ_TILE_Y_K + (k0*QR3_K) % WARP_SIZE], scales,
...
void mul_mat_q_case(const mmq_args & args, cudaStream_t stream) {
    ...
    const int cc   = ggml_cuda_info().devices[id].cc;
    const int smpb = ggml_cuda_info().devices[id].smpb;
    ...
    for (int mmq_x = 8; mmq_x <= mmq_x_max && nwaves_best > 1; mmq_x += 8) {
        ...
        if (nwaves < nwaves_best && mmq_get_shmem(type, mmq_x, mmq_y) <= smpb) {
            mmq_x_best  = mmq_x;
            nwaves_best = nwaves;
        }
    }
