...
typedef void (*load_tiles_mmq_t)(
    const char * __restrict__ x, int * x_tile, const int & kbx0, const int & i_max, const int & stride);
...
#pragma unroll
        for (int l = 0; l < mma_A::ne; ++l) {
            ...
            A[n].x[l] = __vsubss4((x_qs[i*(WARP_SIZE + 1) + k] >> shift) & 0x0F0F0F0F, 0x08080808);
        }
...
#pragma unroll
        for (int l = 0; l < mma_C::ne/2; ++l) {
            ...
            dA[n][l] = x_df[i*(WARP_SIZE/QI4_0) + i/QI4_0 + k0/QI4_0];
        }
    }
...
#pragma unroll
        for (int l = 0; l < mma_A::ne; ++l) {
            const int i     = i0 + n*mma_A::I + mma_A::get_i(l);
            const int k     = k0              + mma_A::get_k(l) % QI4_1;
            const int shift =                4*(mma_A::get_k(l) / QI4_1);

            A[n].x[l] = (x_qs[i*(WARP_SIZE + 1) + k] >> shift) & 0x0F0F0F0F;
        }
...
#pragma unroll
        for (int l = 0; l < mma_C::ne/2; ++l) {
            ...
            dmA[n][l] = x_dm[i*(WARP_SIZE/QI4_0) + i/QI4_1 + k0/QI4_1];
        }
    }
...
#pragma unroll
        for (int l = 0; l < mma_A::ne; ++l) {
            const int i =    i0 + mma_A::get_i(l) + n*mma_A::I;
            const int k = 2*(k0 + mma_A::get_k(l) % QI5_0) + mma_A::get_k(l) / QI5_0;

            A[n].x[l] = x_qs[i*(2*WARP_SIZE + 1) + k];
        }
#pragma unroll
        for (int l = 0; l < mma_C::ne/2; ++l) {
            const int i = i0 + mma_C::get_i(2*l) + n*mma_C::I;

            dA[n][l] = x_df[i*(WARP_SIZE/QI5_0) + i/QI5_0 + k0/QI5_0];
        }
    }
...
#pragma unroll
        for (int l = 0; l < mma_C::ne/2; ++l) {
            ...
            dmA[n][l] = x_dm[i*(WARP_SIZE/QI5_1) + i/QI5_1 + k0/QI5_1];
        }
    }
...
template <int mmq_y, int nwarps, bool need_check> static __device__ __forceinline__ void load_tiles_q8_0_dp4a(
    const char * __restrict__ x, int * __restrict__ x_tile, const int & kbx0, const int & i_max, const int & stride) {
...
template <int mmq_y, int nwarps, bool need_check> static __device__ __forceinline__ void load_tiles_q8_0_mma(
    const char * __restrict__ x, int * __restrict__ x_tile, const int & kbx0, const int & i_max, const int & stride) {
...
#pragma unroll
        for (int l = 0; l < mma_A::ne; ++l) {
            ...
            A[n].x[l] = x_qs[i*MMQ_MMA_TILE_X_K_Q8_0 + k];
        }
...
#pragma unroll
        for (int l = 0; l < mma_A::ne; ++l) {
            ...
            A[n][0].x[l] = (x_qs[i*(WARP_SIZE + 1) + k0 + 0] >> shift) & 0x03030303;
            A[n][1].x[l] = (x_qs[i*(WARP_SIZE + 1) + k0 + 1] >> shift) & 0x03030303;
        }
...
#pragma unroll
            for (int kdm = 0; kdm < 2; ++kdm) {
                const float2 dm = __half22float2(x_dm[i*(WARP_SIZE + 1) + k0 + kdm]);
...
#pragma unroll
        for (int l = 0; l < mma_A::ne; ++l) {
            ...
            A[n][0].x[l] = (x_qs[i*(2*WARP_SIZE + 1) + k/2 + 0]          >> (4*(k%2))) & 0x0F0F0F0F;
            A[n][1].x[l] = (x_qs[i*(2*WARP_SIZE + 1) + k/2 + mma_A::K/2] >> (4*(k%2))) & 0x0F0F0F0F;
...
#pragma unroll
        for (int l = 0; l < mma_C::ne/2; ++l) {
            ...
            dA[n][l] = x_df[i*(WARP_SIZE/QI3_K) + i/QI3_K + k0/QI3_K];
        }
    }
...
#pragma unroll
            for (int l = 0; l < mma_A::ne; ++l) {
                ...
                A[n][kvdr/4].x[l] = (x_qs[i*(WARP_SIZE + 1) + k] >> kvdr) & 0x0F0F0F0F;
            }
...
#pragma unroll
        for (int l = 0; l < mma_C::ne/2; ++l) {
            ...
            dmA[n][l] = x_dm[i*(WARP_SIZE/QI5_K) + i/QI5_K + k0/QI5_K];
        }
    }
...
#pragma unroll
            for (int l = 0; l < mma_A::ne; ++l) {
                ...
                A[n][kvdr/4].x[l] = x_qs[i*(QR5_K*WARP_SIZE + 1) + k];
            }
...
struct mmq_type_traits<mmq_x, mmq_y, nwarps, need_check, GGML_TYPE_Q8_0> {
    ...
    static constexpr load_tiles_mmq_t load_tiles   = load_tiles_q8_0_mma<mmq_y, nwarps, need_check>;
...
static __device__ void mul_mat_q_process_tile(
    ...
    for (int kb0 = kb0_start; kb0 < kb0_stop; kb0 += blocks_per_warp) {
        ...
        load_tiles(x, tile_x_qs, stride01*it*mmq_y + kb0, tile_x_max_i, stride01);
