    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            // store key and value to memory
            {
                ...
                GGML_ASSERT((n_embd_gqa*(il*n_ctx + n_past)) % ggml_blck_size(kv_self.k->type) == 0);
                offload_func_kq(k);
                ggml_set_name(k, "k");
                ...
                GGML_ASSERT(n_ctx  % ggml_blck_size(kv_self.k->type) == 0);
                offload_func_v(v);
                ggml_set_name(v, "v");

                // fprintf(stderr, "N=%d n_past=%d\n", N, n_past);
            ...
            int padded_size = n_past + N + ggml_blck_size(kv_self.v->type) - 1;
            padded_size -= padded_size % ggml_blck_size(kv_self.v->type);

            // split cached V into n_head heads
            const int64_t v_row_size = kv_self.v->type == GGML_TYPE_Q8_0 ? n_ctx + 128 : n_ctx;
            struct ggml_tensor * V =
                ggml_view_3d(ctx0, kv_self.v,
                        padded_size, n_embd_head, n_head_kv,
