...
template <typename T>
static void helper_write(std::stringstream & ss, const T & val) {
    ss.write(reinterpret_cast<const char *>(&val), sizeof(val));
}

static void helper_write(std::stringstream & ss, const void * data, const size_t nbytes) {
    ss.write(reinterpret_cast<const char *>(data), nbytes);
}

static std::stringstream get_handcrafted_file(const unsigned int seed, const enum handcrafted_file_type hft, const int extra_bytes = 0) {
    std::stringstream ss;
    ...
    if (hft == HANDCRAFTED_HEADER_BAD_MAGIC) {
        ...
        helper_write(ss, bad_magic, sizeof(bad_magic));
    } else {
        helper_write(ss, GGUF_MAGIC, 4);
    }

    if (hft == HANDCRAFTED_HEADER_BAD_VERSION_1) {
        const uint32_t version = 1;
        helper_write(ss, version);
    ...
    }
    {
        ...
        helper_write(ss, n_kv);
    }

    if (hft < offset_has_kv) {
        while (size_t(ss.tellp()) % alignment != 0) {
            const char pad = 0;
            helper_write(ss, pad);
        }
    ...
    for (int i = 0; i < int(kv_types.size()); ++i) {
        ...
        helper_write(ss, key.data(), key.length());

        {
            const int32_t type32 = int32_t(type);
            helper_write(ss, type32);
        }
        ...
        if (type == GGUF_TYPE_ARRAY) {
            ...
            helper_write(ss, n);
            helper_write(ss, &data, n*type_size);
            continue;
        }
        ...
        helper_write(ss, data, type_size);
    }
    ...
    if (hft == HANDCRAFTED_KV_BAD_ALIGN      ||
        ...
        helper_write(ss, n);
        helper_write(ss, GGUF_KEY_GENERAL_ALIGNMENT, n);

        const int32_t type = gguf_type(GGUF_TYPE_UINT32);
        helper_write(ss, type);

        alignment = expect_context_not_null(hft) ? 1 : 13;
        helper_write(ss, alignment);
    }
    ...
    for (int i = 0; i < int(tensor_configs.size()); ++i) {
        ...
        helper_write(ss, name.data(), name.length());
        ...
        if (hft == HANDCRAFTED_TENSORS_BAD_OFFSET) {
            ...
            helper_write(ss, bad_offset);
        } else {
            helper_write(ss, offset);
        }
...
static bool handcrafted_check_tensor_data(const gguf_context * gguf_ctx, const unsigned int seed, std::stringstream & ss) {
    if (!gguf_ctx) {
        return false;
    }
    ...
    for (int i = 0; i < int(tensor_configs.size()); ++i) {
        ...
        ss.seekg(gguf_get_data_offset(gguf_ctx) + offset, std::stringstream::beg);
...
static std::pair<int, int> test_handcrafted_file(const unsigned int seed) {
    ...
    for (enum handcrafted_file_type hft : hfts) {
        ...
        std::stringstream ss = get_handcrafted_file(seed, hft);
        ...
        struct gguf_context * gguf_ctx = gguf_init_from_file_impl(ss, gguf_params);
        ...
        if (expect_context_not_null(hft) && hft >= offset_has_data) {
            ...
            if (handcrafted_check_tensor_data(gguf_ctx, seed, ss)) {
                printf("\033[1;32mOK\033[0m\n");
                npass++;
            } else {
                printf("\033[1;31mFAIL\033[0m\n");
...
static std::pair<int, int> test_roundtrip(ggml_backend_dev_t dev, const unsigned int seed, const bool only_meta) {
    ...
    std::stringstream ss;
    gguf_write_to_buf(gguf_ctx_0, ss, only_meta);
    GGML_ASSERT(ss.good());
    ss.seekg(0, std::stringstream::beg);
    ...
    struct gguf_context * gguf_ctx_1 = gguf_init_from_file_impl(ss, gguf_params);
