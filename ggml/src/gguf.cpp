...
template<typename T>
bool gguf_read_emplace_helper(struct gguf_reader & gr, std::vector<struct gguf_kv> & kv, const std::string & key, const size_t n) {
    std::vector<T> value;
    try {
        if (!gr.read(value, n)) {
            return false;
...
struct gguf_context * gguf_init_from_file_impl(std::istream & is, struct gguf_init_params params) {
    ...
    if (ok && gr.read(n_tensors)) {
        if (n_tensors > SIZE_MAX/sizeof(gguf_tensor_info)) {
            fprintf(stderr, "%s: number of tensors too large: %" PRIu64 " > %zu\n",
    ...
    if (ok && gr.read(n_kv)) {
        if (n_kv > SIZE_MAX/sizeof(gguf_kv)) {
            fprintf(stderr, "%s: number of key value pairs too large: %" PRIu64 " > %zu\n",
    ...
    // KV pairs
    {
        for (uint64_t i = 0; ok && i < n_kv; ++i) {
            ...
            try {
                ...
                fprintf(stderr, "%s: encountered length_error while reading key %" PRIu64 "\n", __func__, i);
                ok = false;
            } catch (std::bad_alloc &) {
                fprintf(stderr, "%s: encountered bad_alloc error while reading key %" PRIu64 "\n", __func__, i);
                ok = false;
            }
            ...
            switch (type) {
                case GGUF_TYPE_UINT8:    ok = ok && gguf_read_emplace_helper<uint8_t>    (gr, ctx->kv, key, n); break;
                case GGUF_TYPE_INT8:     ok = ok && gguf_read_emplace_helper<int8_t>     (gr, ctx->kv, key, n); break;
                case GGUF_TYPE_UINT16:   ok = ok && gguf_read_emplace_helper<uint16_t>   (gr, ctx->kv, key, n); break;
                case GGUF_TYPE_INT16:    ok = ok && gguf_read_emplace_helper<int16_t>    (gr, ctx->kv, key, n); break;
                case GGUF_TYPE_UINT32:   ok = ok && gguf_read_emplace_helper<uint32_t>   (gr, ctx->kv, key, n); break;
                case GGUF_TYPE_INT32:    ok = ok && gguf_read_emplace_helper<int32_t>    (gr, ctx->kv, key, n); break;
                case GGUF_TYPE_FLOAT32:  ok = ok && gguf_read_emplace_helper<float>      (gr, ctx->kv, key, n); break;
                case GGUF_TYPE_BOOL:     ok = ok && gguf_read_emplace_helper<bool>       (gr, ctx->kv, key, n); break;
                case GGUF_TYPE_STRING:   ok = ok && gguf_read_emplace_helper<std::string>(gr, ctx->kv, key, n); break;
                case GGUF_TYPE_UINT64:   ok = ok && gguf_read_emplace_helper<uint64_t>   (gr, ctx->kv, key, n); break;
                case GGUF_TYPE_INT64:    ok = ok && gguf_read_emplace_helper<int64_t>    (gr, ctx->kv, key, n); break;
                case GGUF_TYPE_FLOAT64:  ok = ok && gguf_read_emplace_helper<double>     (gr, ctx->kv, key, n); break;
    ...
    for (uint64_t i = 0; ok && i < n_tensors; ++i) {
        ...
        // tensor name
        {
            ...
            try {
                ...
                fprintf(stderr, "%s: encountered length_error while reading tensor name %" PRIu64 "\n", __func__, i);
                ok = false;
            } catch (std::bad_alloc &) {
                fprintf(stderr, "%s: encountered bad_alloc error while reading tensor name %" PRIu64 "\n", __func__, i);
                ok = false;
            }
            ...
            for (uint64_t j = 0; ok && j < i; ++j) {
                if (strcmp(info.t.name, ctx->info[j].t.name) == 0) {
                    fprintf(stderr, "%s: duplicate tensor name '%s'\n", __func__, info.t.name);
                    ok = false;
                    break;
                }
            }
    ...
    if (params.ctx != nullptr) {
        ...
        *params.ctx = ggml_init(pdata);
        ...
        for (uint64_t i = 0; i < n_tensors; ++i) {
            ...
            if (!params.no_alloc) {
                cur->data = (char *) data->data + ctx->info[i].offset;
            }
        }
...
struct gguf_context * gguf_init_from_file(const char * fname, struct gguf_init_params params) {
    ...
    struct gguf_context * result = gguf_init_from_file_impl(ifs, params);
    ifs.close();
    return result;
}
