...
static __global__ void flash_attn_vec_ext_f32(
    ...
    const int j0 = blockIdx.x * ncols1;
    const int c0 = blockIdx.z * ncols2;

    const int gqa_ratio = ne02 / ne12; // With grouped query attention there are > 1 Q matrices per K, V matrix.
    Q += nb02* c0              + nb01*j0;
    K += nb12*(c0 / gqa_ratio);
    V += nb22*(c0 / gqa_ratio); // K and V have same shape
    const half * maskh = (const half   *)  mask + ne11*j0;

    const float slope = get_alibi_slope(max_bias, c0, n_head_log2, m0, m1);
...
#pragma unroll
    for (int jc = 0; jc < ncols; ++jc) {
        KQ[jc*D + tid] = -FLT_MAX/2.0f;
    }

    float kqmax[ncols];
#pragma unroll
    for (int jc = 0; jc < ncols; ++jc) {
        kqmax[jc] = -FLT_MAX/2.0f;
    }
...
#pragma unroll
    for (int jc = 0; jc < ncols; ++jc) {
        if (threadIdx.y == 0) {
            kqmax_shared[jc][threadIdx.x] = -FLT_MAX/2.0f;
...
#pragma unroll
            for (int i0 = 0; i0 < D/2; i0 += WARP_SIZE) {
                ...
                Q_f2[jc][i0/WARP_SIZE]    = ncols1 <= 2 || j0 + j < ne01 ? Q_f2_j[i] : make_float2(0.0f, 0.0f);
...
#pragma unroll
            for (int i0 = 0; i0 < D; i0 += WARP_SIZE) {
                skip = skip && isinf(maskf_shared[j*D + tid]);
            }
        }
...
#pragma unroll
        for (int jc = 0; jc < ncols; ++jc) {
            float kqmax_new_j = kqmax_new_arr[jc];

            if (threadIdx.x == 0) {
                kqmax_shared[jc][threadIdx.y] = kqmax_new_j;
            }
        }
...
#pragma unroll
        for (int jc = 0; jc < ncols; ++jc) {
            float kqmax_new_j = kqmax_shared[jc][threadIdx.x];
            kqmax_new_j = warp_reduce_max(kqmax_new_j);

            const float KQ_max_scale = expf(kqmax[jc] - kqmax_new_j);
            kqmax[jc] = kqmax_new_j;
...
#pragma unroll
    for (int jc_VKQ = 0; jc_VKQ < ncols; ++jc_VKQ) {
        ...
        const int j_dst = (j0 + j_VKQ)*gridDim.y + blockIdx.y;
        const int c_dst =  c0 + c_VKQ;
        dst[j_dst*gridDim.z*(D*ncols2) + D*c_dst + tid] = dst_val;
    }

    if (gridDim.y != 1 && tid < ncols && (ncols1 <= 2 || j0 + tid < ne01)) {
        const int j = j0 + tid / ncols2;
        const int c = c0 + tid % ncols2;

        dst_meta[(j*gridDim.z*ncols2 + c) * gridDim.y + blockIdx.y] = make_float2(kqmax[tid], kqsum[tid]);
    }
#else
    GGML_UNUSED(Q); GGML_UNUSED(K); GGML_UNUSED(V); GGML_UNUSED(mask);
    GGML_UNUSED(dst); GGML_UNUSED(dst_meta); GGML_UNUSED(scale);
