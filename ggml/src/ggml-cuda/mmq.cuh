...
#define MMQ_DP4A_TXS_Q3_K tile_x_sizes{mmq_y*WARP_SIZE*2 + mmq_y, mmq_y*WARP_SIZE/2       + mmq_y/2,         0}
...
static constexpr __host__ __device__ tile_x_sizes mmq_get_dp4a_tile_x_sizes(ggml_type type, int mmq_y) {
    return type == GGML_TYPE_Q4_0 ? MMQ_DP4A_TXS_Q8_0 :
        type == GGML_TYPE_Q4_1    ? MMQ_DP4A_TXS_Q8_1 :
...
#define MMQ_MMA_TILE_X_K_Q8_1 (2*WARP_SIZE + 2*WARP_SIZE/QI8_1                 + 4)
...
#else
        x_qs[i*(WARP_SIZE + 1)       + kbx*(2*QI4_0) + kqsx + 0]     = __vsubss4((qs0 >> 0) & 0x0F0F0F0F, 0x08080808);
        x_qs[i*(WARP_SIZE + 1)       + kbx*(2*QI4_0) + kqsx + QI4_1] = __vsubss4((qs0 >> 4) & 0x0F0F0F0F, 0x08080808);
#endif // INT8_MMA_AVAILABLE
    }
...
#else
        x_qs[i*(WARP_SIZE + 1)       + kbx*(2*QI4_1) + kqsx + 0]     = (qs0 >> 0) & 0x0F0F0F0F;
        x_qs[i*(WARP_SIZE + 1)       + kbx*(2*QI4_1) + kqsx + QI4_1] = (qs0 >> 4) & 0x0F0F0F0F;
#endif // INT8_MMA_AVAILABLE
    }
...
#pragma unroll
            for (int l = 0; l < mma_C::ne/2; ++l) {
                ...
                dsB[l] = __half22float2(y_ds[j*MMQ_TILE_Y_K + (k0/QI8_1) % (WARP_SIZE/QI8_1)]);
            }
...
#pragma unroll
                for (int l = 0; l < mma_C::ne; ++l) {
                    sum[(j0/mma_C::J + n)*mma_C::ne + l] += dmA[n][l/2][k01/QI8_1].x*dsB[l%2].x*C.x[l] + dmA[n][l/2][k01/QI8_1].y*dsB[l%2].y;
                }
            }
        }
    }
...
#else
            x_df[i*(WARP_SIZE/2) + i/2   + sizeof(int)*(threadIdx.x % (WARP_SIZE/8)) + l] = d*sc8[l];
#endif // INT8_MMA_AVAILABLE
        }
    }
}
...
#pragma unroll
            for (int i0 = 0; i0 < mmq_y; i0 += WARP_SIZE) {
                ...
                sum[j0/nwarps*mmq_y/WARP_SIZE + i0/WARP_SIZE] += vec_dot_q3_K_q8_1_impl_mmq(
                    &x_qs[i*(2*WARP_SIZE + 1) + k0], &y_qs[j*MMQ_TILE_Y_K + k01],
                    &x_df[i*(WARP_SIZE/2) + i/2 + k0/4], y_df[j*MMQ_TILE_Y_K + k01/QI8_1]);
            }
        }
    }
}
