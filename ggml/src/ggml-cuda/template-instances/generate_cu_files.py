...
for ncols in [8, 16, 24, 32, 64]:
    for ncols2 in [1, 2, 4, 8, 16, 24, 32]:
        if ncols2 > ncols:
            continue
        if (ncols == 24) != (ncols2 == 24):
            continue
        ncols1 = ncols // ncols2
        with open(f"fattn-mma-f16-instance-ncols1_{ncols1}-ncols2_{ncols2}.cu", "w") as f:
            ...
            for head_size_kq in HEAD_SIZES_KQ:
                ...
                if head_size_kq == 72:
                    continue
                if head_size_kq != 576 and ncols2 in (16, 24, 32):
                    continue
                if head_size_kq == 576 and ncols2 not in (4, 16, 24, 32):
                    continue
                head_size_v = head_size_kq if head_size_kq != 576 else 512
