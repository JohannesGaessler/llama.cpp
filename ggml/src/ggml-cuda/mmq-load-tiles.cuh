...
template <ggml_type type, int J, bool fallback> static __device__ __forceinline__ void ggml_cuda_mmq_load_tiles_q8_0_repacked(
        const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int warp_size   = ggml_cuda_get_physical_warp_size();
    constexpr int nwarps      = ggml_cuda_mmq_get_nthreads(type, J, fallback) / warp_size;
    constexpr int I           = ggml_cuda_mmq_get_I(type, J, fallback);
...

    {
        ...
        half2 tmph2;
        ggml_cuda_memcpy_1<4>(&tmph2, &bxi->d[i % 64][(threadIdx.x % 2) * 2]);
        const float2 tmpf2 = __half22float2(tmph2);
        ggml_cuda_memcpy_1<8>(x_df + i*sram_stride + (threadIdx.x % 2) * 2, &tmpf2);
    }
    {
