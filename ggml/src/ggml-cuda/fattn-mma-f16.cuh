...
static __global__ void flash_attn_ext_f16(
    ...
    int       kbc      = (blockIdx.x + 0)*(iter_k*ne03*iter_j*(ne02/ncols2)) / gridDim.x;
    const int kbc_stop = (blockIdx.x + 1)*(iter_k*ne03*iter_j*(ne02/ncols2)) / gridDim.x;
    ...
    while (kbc < kbc_stop && kb0_stop == iter_k) {
        const int head = kbc / (iter_k*ne03*iter_j);
        const int jt = (kbc - iter_k*ne03*iter_j*head) / (iter_k*ne03); // j index of current tile.
        const int sequence = (kbc - iter_k*ne03*iter_j*head - iter_k*ne03*jt) / iter_k;
