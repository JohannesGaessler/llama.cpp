...
static constexpr __host__ __device__ fattn_mma_config ggml_cuda_fattn_mma_get_config_rdna3(const int DKQ, const int DV, const int ncols) {
    GGML_CUDA_FATTN_MMA_CONFIG_CASE(128, 128, 32,  32, 2,  64,  64,  64,  64, 2, true);
...
#pragma unroll
                    for (int l = 0; l < T_C_VKQ::J; ++l) {
                        tile_Q_h[j*(2*tile_stride) + 2*k1 + 2*l + threadIdx.x/16] = ((const half *) VKQ_C[(k00 + k1)/T_C_VKQ::J].x)[2*l + 0];
                    }
                }
                {
...
#pragma unroll
                    for (int l = 0; l < T_C_VKQ::J; ++l) {
                        tile_Q_h[j*(2*tile_stride) + 2*k1 + 2*l + threadIdx.x/16] = ((const half *) VKQ_C[(k00 + k1)/T_C_VKQ::J].x)[2*l + 1];
                    }
                }
#else
#pragma unroll
