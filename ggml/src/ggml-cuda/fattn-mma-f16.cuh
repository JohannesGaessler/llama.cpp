...
template<int ncols1, int nwarps, int KQ_stride>
static __device__ __forceinline__ void flash_attn_ext_f16_load_mask(
        const half2 * const __restrict__ mask_h2, half2 * const __restrict__ tile_mask, const int stride_mask) {
    static_assert(KQ_stride == 2*WARP_SIZE || KQ_stride == WARP_SIZE, "bad KQ_stride");
#ifdef CP_ASYNC_AVAILABLE
    constexpr int preload = 2*KQ_stride;
    constexpr int cols_per_warp = 8*WARP_SIZE/KQ_stride;
...
#pragma unroll
    for (int j0 = 0; j0 < ncols1; j0 += stride_j) {
        const int j = j0 + threadIdx.y*cols_per_warp +
            (KQ_stride == 2*WARP_SIZE ? threadIdx.x / (WARP_SIZE/4) : threadIdx.x / (WARP_SIZE/8));

        if (j0 + stride_j > ncols1 && j >= ncols1) {
            break;
        }

        const int i = 4 * (KQ_stride == 2*WARP_SIZE ? threadIdx.x % (WARP_SIZE/4) : threadIdx.x % (WARP_SIZE/8));

        cp_async_cg_16<preload>(tile_mask_32 + j*(KQ_stride*sizeof(half) + 16) + i*sizeof(half2), mask_h2 + j*stride_mask + i);
    }
#else
    constexpr int cols_per_warp = 2*WARP_SIZE/KQ_stride;
    constexpr int stride_j = nwarps * cols_per_warp;
#pragma unroll
    for (int j0 = 0; j0 < ncols1; j0 += stride_j) {
        const int j = j0 + threadIdx.y*cols_per_warp + (KQ_stride == 2*WARP_SIZE ? 0 : threadIdx.x / (WARP_SIZE/2));

        if (j0 + stride_j > ncols1 && j >= ncols1) {
            break;
        }

        const int i = KQ_stride == 2*WARP_SIZE ? threadIdx.x : threadIdx.x % (WARP_SIZE/2);

        tile_mask[j*(KQ_stride/2 + 4) + i] = mask_h2[j*stride_mask + i];
    }
#endif // CP_ASYNC_AVAILABLE
}

template<int D, int ncols1, int ncols2, int nwarps, int KQ_stride, bool use_logit_softcap, bool needs_fixup, bool is_fixup, bool last_iter>
static __device__ __forceinline__ void flash_attn_ext_f16_iter(
    ...
    constexpr int np = nwarps*(tile_B::I/ncols2) / ncols1; // Number of parallel CUDA warps per Q column.
    constexpr int D2_padded = D/2 + 4; // Size of D in half2, padded to avoid shared memory bank conflicts.
...
#else
    if (ncols2 > 1 || mask_h2) {
        flash_attn_ext_f16_load_mask<ncols1, nwarps, KQ_stride>(mask_h2 + k_VKQ_0/2, tile_mask, stride_mask);
    }
...
#pragma unroll
            for (int l = 0; l < tile_C_KQ::ne; ++l) {
                ...
                KQ_C[i00/(np*tile_C_KQ::I)].x[l] += slope*__half2float(((const half *) tile_mask)[j*(KQ_stride + 8) + i]);
            }
        }
    }
...
#pragma unroll
    ...
    if (!last_iter) {
        if (ncols2 > 1 || mask_h2) {
            flash_attn_ext_f16_load_mask<ncols1, nwarps, KQ_stride>(mask_h2 + (k_VKQ_0 + KQ_stride)/2, tile_mask, stride_mask);
        }
...
template<int D, int ncols1, int ncols2, int nwarps, int KQ_stride, bool use_logit_softcap, bool needs_fixup, bool is_fixup>
static __device__ __forceinline__ void flash_attn_ext_f16_process_tile(
    ...
    static_assert(nwarps*(tile_B::I/ncols2) % ncols1 == 0, "bad nwarps");
    constexpr int np = nwarps*(tile_B::I/ncols2) / ncols1; // Number of parallel CUDA warps per Q column.
    ...
    half2 * tile_V    = tile_K;
#endif // CP_ASYNC_AVAILABLE
    half2 * tile_mask = tile_V + KQ_stride*D2_padded;
...
#pragma unroll
        for (int jc0 = 0; jc0 < ncols; jc0 += nwarps*stride_j) {
            const int jc = jc0 + threadIdx.y*stride_j + (stride_k == WARP_SIZE ? 0 : threadIdx.x / stride_k);

            if (jc0 + nwarps*stride_j > ncols && jc >= ncols) {
                break;
            }
...
#pragma unroll
                for (int k0 = k0_start; k0 < k0_stop; k0 += stride_k) {
                    ...
                    tile_K[(j*ncols2 + c)*D2_padded + k] = scale_h2 * make_half2(tmp.x, tmp.y);
                }
            } else {
#pragma unroll
                for (int k0 = k0_start; k0 < k0_stop; k0 += stride_k) {
                    ...
                    tile_K[(j*ncols2 + c)*D2_padded + k] = make_half2(0.0f, 0.0f);
                }
            }
        }
    }
...
#pragma unroll
...
#ifdef CP_ASYNC_AVAILABLE
    if (ncols2 > 1 || mask_h2) {
        flash_attn_ext_f16_load_mask<ncols1, nwarps, KQ_stride>(mask_h2 + kb0_start*KQ_stride/2, tile_mask, stride_mask);
    }
    ...
    for (int kb0 = kb0_start; kb0 < kb0_stop-1; ++kb0) {
        ...
        flash_attn_ext_f16_iter<D, ncols1, ncols2, nwarps, KQ_stride, use_logit_softcap, needs_fixup, is_fixup, last_iter>
...
#pragma unroll
            for (int jc0_dst = 0; jc0_dst < ncols; jc0_dst += (nwarps/np)*stride_j) {
                const int jc_dst = jc0_dst + (threadIdx.y/np)*stride_j + (stride_k == WARP_SIZE ? 0 : threadIdx.x / stride_k);

                if (jc0_dst + (nwarps/np)*stride_j > ncols && jc_dst >= ncols) {
                    break;
                }

                const int jc_tile_K = (jc_dst/tile_B::I)*(np*tile_B::I) + jc_dst % tile_B::I;
...
#pragma unroll
                    for (int ip = 0; ip < np; ++ip) {
                        ...
                        const float2 dstk_val_add = __half22float2(tile_K[(jc_tile_K + ip*tile_B::I)*D2_padded + k]);
...
template<int D, int ncols1, int ncols2, int nwarps, int KQ_stride, bool use_logit_softcap>
...
static __global__ void flash_attn_ext_f16(
    ...
    constexpr int kb_niter = FATTN_KQ_STRIDE / KQ_stride; // Number of kernel iterations per assigned KQ slice.
    ...
    while (kbc < kbc_stop && kb0_stop == iter_k) {
        ...
        if (kb0_start == 0) {
            ...
            flash_attn_ext_f16_process_tile<D, ncols1, ncols2, nwarps, KQ_stride, use_logit_softcap, needs_fixup, is_fixup>
...
void ggml_cuda_flash_attn_ext_mma_f16_case(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    static_assert(ncols % tile_B::I == 0, "bad ncols");
    ...
    constexpr int nwarps    = (KQ_stride == 32 && ncols <= 16) ? ncols/tile_B::J * KQ_stride/tile_A::I : (ncols <= 8 ? 4 : 8);

    const int nrows_KQ = cp_async_available(cc) ? 2*KQ_stride : KQ_stride;

    const size_t nbytes_shared_KV      = nrows_KQ         * (D        + 8) * sizeof(half);
    const size_t nbytes_shared_mask    = ncols1           * (nrows_KQ + 8) * sizeof(half);
    const size_t nbytes_shared_combine = nwarps*tile_B::J * (D        + 8) * sizeof(half);
    const size_t nbytes_shared = std::max(nbytes_shared_KV + nbytes_shared_mask, nbytes_shared_combine);
    ...
    if (logit_softcap == 0.0f) {
        ...
        fattn_kernel = flash_attn_ext_f16<D, ncols1, ncols2, nwarps, KQ_stride, use_logit_softcap>;
    } else {
    ...
    if (!shared_memory_limit_raised[id]) {
        CUDA_CHECK(cudaFuncSetAttribute(flash_attn_ext_f16<D, ncols1, ncols2, nwarps, KQ_stride, false>,
            cudaFuncAttributeMaxDynamicSharedMemorySize, nbytes_shared));
        CUDA_CHECK(cudaFuncSetAttribute(flash_attn_ext_f16<D, ncols1, ncols2, nwarps, KQ_stride, true>,
            cudaFuncAttributeMaxDynamicSharedMemorySize, nbytes_shared));
        shared_memory_limit_raised[id] = true;
    }
#endif // !(defined(GGML_USE_HIP) && defined(__HIP_PLATFORM_AMD__))

    launch_fattn<D, ncols1, ncols2, 0, KQ_stride>(ctx, dst, fattn_kernel, nwarps, nbytes_shared, true, true);
}
...
extern DECL_FATTN_MMA_F16_CASE( 64,  8, 1);
extern DECL_FATTN_MMA_F16_CASE( 80,  8, 1);
extern DECL_FATTN_MMA_F16_CASE( 96,  8, 1);
extern DECL_FATTN_MMA_F16_CASE(112,  8, 1);
extern DECL_FATTN_MMA_F16_CASE(128,  8, 1);
extern DECL_FATTN_MMA_F16_CASE(256,  8, 1);

extern DECL_FATTN_MMA_F16_CASE( 64,  4, 2);
extern DECL_FATTN_MMA_F16_CASE( 80,  4, 2);
extern DECL_FATTN_MMA_F16_CASE( 96,  4, 2);
extern DECL_FATTN_MMA_F16_CASE(112,  4, 2);
extern DECL_FATTN_MMA_F16_CASE(128,  4, 2);
extern DECL_FATTN_MMA_F16_CASE(256,  4, 2);

extern DECL_FATTN_MMA_F16_CASE( 64,  2, 1);
extern DECL_FATTN_MMA_F16_CASE( 80,  2, 1);
extern DECL_FATTN_MMA_F16_CASE( 96,  2, 1);
extern DECL_FATTN_MMA_F16_CASE(112,  2, 1);
extern DECL_FATTN_MMA_F16_CASE(128,  2, 1);
extern DECL_FATTN_MMA_F16_CASE(256,  2, 1);

extern DECL_FATTN_MMA_F16_CASE( 64,  1, 8);
extern DECL_FATTN_MMA_F16_CASE( 80,  1, 8);
extern DECL_FATTN_MMA_F16_CASE( 96,  1, 8);
extern DECL_FATTN_MMA_F16_CASE(112,  1, 8);
extern DECL_FATTN_MMA_F16_CASE(128,  1, 8);
extern DECL_FATTN_MMA_F16_CASE(256,  1, 8);

extern DECL_FATTN_MMA_F16_CASE( 64, 16, 1);
extern DECL_FATTN_MMA_F16_CASE( 80, 16, 1);
extern DECL_FATTN_MMA_F16_CASE( 96, 16, 1);
extern DECL_FATTN_MMA_F16_CASE(112, 16, 1);
extern DECL_FATTN_MMA_F16_CASE(128, 16, 1);
extern DECL_FATTN_MMA_F16_CASE(256, 16, 1);

extern DECL_FATTN_MMA_F16_CASE( 64,  8, 2);
extern DECL_FATTN_MMA_F16_CASE( 80,  8, 2);
extern DECL_FATTN_MMA_F16_CASE( 96,  8, 2);
extern DECL_FATTN_MMA_F16_CASE(112,  8, 2);
extern DECL_FATTN_MMA_F16_CASE(128,  8, 2);
extern DECL_FATTN_MMA_F16_CASE(256,  8, 2);

extern DECL_FATTN_MMA_F16_CASE( 64,  4, 4);
extern DECL_FATTN_MMA_F16_CASE( 80,  4, 4);
extern DECL_FATTN_MMA_F16_CASE( 96,  4, 4);
extern DECL_FATTN_MMA_F16_CASE(112,  4, 4);
extern DECL_FATTN_MMA_F16_CASE(128,  4, 4);
extern DECL_FATTN_MMA_F16_CASE(256,  4, 4);

extern DECL_FATTN_MMA_F16_CASE( 64,  2, 8);
extern DECL_FATTN_MMA_F16_CASE( 80,  2, 8);
extern DECL_FATTN_MMA_F16_CASE( 96,  2, 8);
extern DECL_FATTN_MMA_F16_CASE(112,  2, 8);
extern DECL_FATTN_MMA_F16_CASE(128,  2, 8);
extern DECL_FATTN_MMA_F16_CASE(256,  2, 8);

extern DECL_FATTN_MMA_F16_CASE( 64, 32, 1);
extern DECL_FATTN_MMA_F16_CASE( 80, 32, 1);
extern DECL_FATTN_MMA_F16_CASE( 96, 32, 1);
extern DECL_FATTN_MMA_F16_CASE(112, 32, 1);
extern DECL_FATTN_MMA_F16_CASE(128, 32, 1);
extern DECL_FATTN_MMA_F16_CASE(256, 32, 1);

extern DECL_FATTN_MMA_F16_CASE( 64, 16, 2);
extern DECL_FATTN_MMA_F16_CASE( 80, 16, 2);
extern DECL_FATTN_MMA_F16_CASE( 96, 16, 2);
extern DECL_FATTN_MMA_F16_CASE(112, 16, 2);
extern DECL_FATTN_MMA_F16_CASE(128, 16, 2);
extern DECL_FATTN_MMA_F16_CASE(256, 16, 2);

extern DECL_FATTN_MMA_F16_CASE( 64,  8, 4);
extern DECL_FATTN_MMA_F16_CASE( 80,  8, 4);
extern DECL_FATTN_MMA_F16_CASE( 96,  8, 4);
extern DECL_FATTN_MMA_F16_CASE(112,  8, 4);
extern DECL_FATTN_MMA_F16_CASE(128,  8, 4);
extern DECL_FATTN_MMA_F16_CASE(256,  8, 4);

extern DECL_FATTN_MMA_F16_CASE( 64,  4, 8);
extern DECL_FATTN_MMA_F16_CASE( 80,  4, 8);
extern DECL_FATTN_MMA_F16_CASE( 96,  4, 8);
extern DECL_FATTN_MMA_F16_CASE(112,  4, 8);
extern DECL_FATTN_MMA_F16_CASE(128,  4, 8);
extern DECL_FATTN_MMA_F16_CASE(256,  4, 8);

extern DECL_FATTN_MMA_F16_CASE( 64, 64, 1);
extern DECL_FATTN_MMA_F16_CASE( 80, 64, 1);
extern DECL_FATTN_MMA_F16_CASE( 96, 64, 1);
extern DECL_FATTN_MMA_F16_CASE(112, 64, 1);
extern DECL_FATTN_MMA_F16_CASE(128, 64, 1);
extern DECL_FATTN_MMA_F16_CASE(256, 64, 1);

extern DECL_FATTN_MMA_F16_CASE( 64, 32, 2);
extern DECL_FATTN_MMA_F16_CASE( 80, 32, 2);
extern DECL_FATTN_MMA_F16_CASE( 96, 32, 2);
extern DECL_FATTN_MMA_F16_CASE(112, 32, 2);
extern DECL_FATTN_MMA_F16_CASE(128, 32, 2);
extern DECL_FATTN_MMA_F16_CASE(256, 32, 2);

extern DECL_FATTN_MMA_F16_CASE( 64, 16, 4);
extern DECL_FATTN_MMA_F16_CASE( 80, 16, 4);
extern DECL_FATTN_MMA_F16_CASE( 96, 16, 4);
extern DECL_FATTN_MMA_F16_CASE(112, 16, 4);
extern DECL_FATTN_MMA_F16_CASE(128, 16, 4);
extern DECL_FATTN_MMA_F16_CASE(256, 16, 4);

extern DECL_FATTN_MMA_F16_CASE( 64,  8, 8);
extern DECL_FATTN_MMA_F16_CASE( 80,  8, 8);
extern DECL_FATTN_MMA_F16_CASE( 96,  8, 8);
extern DECL_FATTN_MMA_F16_CASE(112,  8, 8);
extern DECL_FATTN_MMA_F16_CASE(128,  8, 8);
extern DECL_FATTN_MMA_F16_CASE(256,  8, 8);
