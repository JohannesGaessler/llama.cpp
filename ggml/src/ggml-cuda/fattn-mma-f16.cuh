...
static __device__ __forceinline__ void flash_attn_ext_f16_load_tile_async_loop_iter_async(
        const int n, const half2 * const __restrict__ KV, const unsigned int tile_KV_32, const int chunks_per_row, const int stride_KV) {
...
#pragma unroll
        ...
        ggml_cuda_unroll<5>{}(load, KV, tile_KV_32, chunks_per_row, stride_KV);
    } else {
