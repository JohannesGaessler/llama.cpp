...
static constexpr __host__ __device__ int calc_rows_per_block(int ncols_dst, int table_id, bool small_k = false) {
    if (small_k) {
        return calc_nwarps(ncols_dst, (mmvq_parameter_table_id)table_id);
    }
...
static __global__ void mul_mat_vec_q(
    ...
    constexpr int rows_per_cuda_block = calc_rows_per_block(ncols_dst, table_id, small_k);
...
static std::pair<dim3, dim3> calc_launch_params(
    ...
    const int64_t nblocks = (nrows_x + calc_rows_per_block(ncols_dst, table_id, small_k) - 1) / calc_rows_per_block(ncols_dst, table_id, small_k);
