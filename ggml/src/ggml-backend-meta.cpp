...
static enum ggml_status ggml_backend_meta_buffer_init_tensor(ggml_backend_buffer_t buffer, ggml_tensor * tensor) {
    ...
    for (size_t j = 0; j < buf_ctx->buf_configs.size(); j++) {
        ...
        for (int i = 0; i < GGML_MAX_DIMS; i++) {
            t_ij->nb[i] = split_dim < 0 || split_dim >= GGML_MAX_DIMS || tensor->nb[i] <= tensor->nb[split_dim] ?
                tensor->nb[i] : tensor->nb[i] * t_ij->ne[split_dim]/tensor->ne[split_dim];
        }
    ...
    size_t max_tmp_size = 0;
    {
        ...
        for (int i = 0; i < cgraph->n_nodes; i++) {
            ...
            if (i + 1 == cgraph->n_nodes) {
                ...
                const ggml_backend_meta_split_state split_state_unfixed = ggml_backend_meta_get_split_state(node, false, false);
        ...
        if (i < n_subgraphs - 1) {
            ...
            for (size_t offset_j = 1; offset_j < n_backends; offset_j *= 2) {
                for (size_t j = 0; j < n_backends; j++) {
                    ...
                    ggml_tensor * node_tmp = ggml_dup_tensor(bcj.ctx, node);
                    node_tmp->buffer = bcj.buf;
                    node_tmp->data = ggml_backend_buffer_get_base(bcj.buf);
                    bcj.cgraphs[i].nodes_aux.push_back(node_tmp);

                    ggml_backend_tensor_copy_async(bcj_other.backend, bcj.backend, node_other, node_tmp);
