...
struct ggml_backend_meta_context {
    ...
    struct backend_config {
        ...
        ggml_backend_buffer_t        bufs[2] = {nullptr, nullptr}; // Multiple buffers to reduce synchronizations.
...
static enum ggml_status ggml_backend_meta_graph_compute(ggml_backend_t backend, struct ggml_cgraph * cgraph) {
    ...
    for (size_t j = 0; j < n_backends; j++) {
        ...
        bcj.cgraphs.clear();
        bcj.nodes.clear();
        bcj.nodes.reserve(cgraph->n_nodes*n_backends);
    ...
    ggml_init_params params = {
        /*.mem_size   =*/ n_subgraphs*(n_backends-1)*2*ggml_tensor_overhead(),
        /*.mem_buffer =*/ nullptr,
        /*.no_alloc   =*/ true,
    };
    ...
    auto allreduce_fallback = [&](size_t i) -> ggml_status {
        size_t i_buf = i % 2; // Alternate between tmp buffers per simple backend to reduce synchronizations.
        auto & bcj_red = backend_ctx->backend_configs[j_reduce];
        ggml_tensor * node_red = bcj_red.cgraphs[i].cgraph_main.nodes[bcj_red.cgraphs[i].cgraph_main.n_nodes-1];
        GGML_ASSERT(ggml_is_contiguous(node_red));
        ...
        for (size_t j = 0; j < n_backends; j++) {
            ...
            bcj.cgraphs[i].nodes_aux.reserve(n_backends - 1);
        }
        ...
        for (size_t j = 0; j < n_backends; j++) {
            ...
            auto & bcj_src = backend_ctx->backend_configs[j];
            ...
            ggml_tensor * node_tmp = ggml_dup_tensor(bcj_red.ctx, node_src);
            node_tmp->buffer = bcj_red.bufs[i_buf];
            node_tmp->data = ggml_backend_buffer_get_base(bcj_red.bufs[i_buf]);
            bcj_red.cgraphs[i].nodes_aux.push_back(node_tmp);

            ggml_backend_tensor_copy_async(bcj_src.backend, bcj_red.backend, node_src, node_tmp);

            ggml_tensor * node_add = ggml_add_inplace(bcj_red.ctx, node_red, node_tmp);
            node_add->flags |= GGML_TENSOR_FLAG_COMPUTE;
            ggml_backend_view_init(node_add);
            bcj_red.cgraphs[i].nodes_aux.push_back(node_add);
        }

        bcj_red.cgraphs[i].cgraph_aux         = *cgraph;
        bcj_red.cgraphs[i].cgraph_aux.nodes   = bcj_red.cgraphs[i].nodes_aux.data();
        bcj_red.cgraphs[i].cgraph_aux.n_nodes = bcj_red.cgraphs[i].nodes_aux.size();

        const ggml_status status = ggml_backend_graph_compute_async(bcj_red.backend, &bcj_red.cgraphs[i].cgraph_aux);
        ...
        for (size_t j = 0; j < n_backends; j++) {
            ...
            ggml_backend_tensor_copy_async(bcj_red.backend, bcj_dst.backend, node_red, node_dst);
        }
        j_reduce = (j_reduce + 1) % n_backends;
        return GGML_STATUS_SUCCESS;
    };
