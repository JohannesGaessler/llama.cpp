...
struct ggml_backend_meta_context {
    ...
    ggml_backend_meta_context(ggml_backend_dev_t meta_dev, const char * params) {
        ...
        for (size_t i = 0; i < n_devs; i++) {
            ...
            backend_configs.emplace_back(ggml_backend_dev_init(simple_dev, params), n_reduce_steps);
        }
        name += ")";
    }
...
static enum ggml_status ggml_backend_meta_graph_compute(ggml_backend_t backend, struct ggml_cgraph * cgraph) {
    ...
    if (max_nnodes_raised || n_subgraphs > backend_ctx->max_subgraphs) {
        ...
        const size_t n_nodes_per_device = 2 * backend_ctx->n_reduce_steps; // tmp + ADD per step
        const size_t n_cgraphs_per_device = 1 + backend_ctx->n_reduce_steps;    // (1 for zeroing +) 1 ADD graph per step
    ...
    auto set_tmp_data = [&](ggml_tensor * tensor, const size_t j, const size_t i_buf) {
        ...
        if (!buf_ptr || ggml_backend_buffer_get_size(buf_ptr.get()) < max_tmp_size) {
            buf_ptr.reset(ggml_backend_alloc_buffer(bcj.backend, max_tmp_size));
        }
